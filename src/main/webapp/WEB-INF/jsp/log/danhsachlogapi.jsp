							...
							<tbody>
								<c:forEach items="${logApis}" var="item" varStatus="status">
									<tr>
										...
										<td style="padding-left: 5px;"><a href="javascript:void(0)" onclick="loadEdit('${contextPath}/danh-sach-log-api/xem?code=<c:out value="${item.code }"/>&id=${item.id}&logId=<c:out value="${item.logId}"/>&time=<fmt:formatDate pattern = "yyyy-MM-dd" value = "${item.date }" />')" data-toggle="modal" data-target="#largeModal" class="text-info"><c:out value="${item.uri }"/></a></td>
										<td>
...
<%-- 													<td>${item.logId }</td> --%>
										...
										<td class="text-center">
											<a href="javascript:void(0)" onclick="loadEdit('${contextPath}/danh-sach-log-api/xem?code=<c:out value="${item.code }"/>&id=${item.id}&logId=<c:out value="${item.logId}"/>&time=<fmt:formatDate pattern = "yyyy-MM-dd" value = "${item.date }" />')" data-toggle="modal" data-target="#largeModal" class="text-info">
												<i class="fa fa-eye"></i>
											</a>
											...
											<a style="margin-left: 5px;" href="javascript:void(0)" onclick="loadEdit('${contextPath}/danh-sach-log-api/unzip?code=<c:out value="${item.code }"/>&id=${item.id}&time=<fmt:formatDate pattern = "yyyy-MM-dd" value = "${item.date }" />')" data-toggle="modal" data-target="#largeModal" class="text-info">
